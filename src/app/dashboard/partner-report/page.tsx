...
export default function PartnerReportPage() {
  ...
  const [showCmaDropdown, setShowCmaDropdown] = useState<boolean>(false);
  ...
  return (
    <div className="min-h-screen max-w-full overflow-x-hidden bg-black text-zinc-100 p-3 sm:p-8 print:p-0 print:bg-white print:text-black">
      ...
      <div className="mb-10">
        <div 
          onClick={() => setShowCmaDropdown(!showCmaDropdown)}
          className="flex items-center justify-between p-4 rounded-2xl bg-zinc-900/80 border border-amber-500/30 shadow-lg cursor-pointer hover:border-amber-500/50 transition-all"
        >
          <div>
            <h2 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
              <ShieldCheck size={18} className="text-amber-400" />
              <span>Closed Month Audit (CMA) History & Bank Transfers</span>
              {auditHistory.length > 0 && (
                <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">
                  {auditHistory.length} Month{auditHistory.length > 1 ? 's' : ''} Closed
                </span>
              )}
            </h2>
            <p className="text-xs text-zinc-400 mt-0.5">Historical archive of finalized month closures, bank reserve floats kept, and partner payouts transferred.</p>
          </div>
          <button 
            type="button"
            className="flex items-center gap-2 text-xs text-amber-300 font-extrabold bg-amber-500/20 hover:bg-amber-500/30 px-4 py-2.5 rounded-xl border border-amber-500/30 transition-all shrink-0 ml-3 shadow-md"
          >
            <span>{showCmaDropdown ? 'Hide CMA' : 'View CMA (Closed Month Audit)'}</span>
            <ChevronDown size={16} className={`transition-transform duration-300 ${showCmaDropdown ? 'rotate-180' : ''}`} />
          </button>
        </div>
          ...
          <div className="mt-4 space-y-4 animate-fade-in">
            {auditHistory.length === 0 ? (
              ...
              auditHistory.map((item) => (
                <div key={item.id} className="bg-zinc-900/90 border border-amber-500/20 p-5 rounded-2xl space-y-4 backdrop-blur-xl shadow-xl">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-3">
                    <div>
